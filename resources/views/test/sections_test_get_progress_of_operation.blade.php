@extends("test.layout_for_different_tests")
...
@section("tag_head")
	<head>
		...
		<title>{{$page_title ?? "TEST GET PROGRESS OF OPERATION"}}</title>
		...
		<script src="{{asset('/js/main__test_get_progress_of_operation.js')}}" type="module"></script>
		{{--	for debugging --}}
...
@section("content")
	{{-- CONTENT --}}
	<content>
		@include("test.content_test_get_progress_of_operation")
	</content>
@endsection
